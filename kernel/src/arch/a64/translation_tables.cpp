...
struct [[gnu::packed]] ARMv8MMU_Upper_Entry_Block {
    u64 descriptor_code : 2, lower_attrs : 10, res0_0 : 4, nT : 1, res0_1 : 4, address : 27, res0_2 : 4,
        upper_attrs : 12;
};
...
bool crude_map_region(uPtr virt_addr, uPtr phys_addr, uSize size, u64 flags, uPtr tables_start, uPtr& tables_current,
                      uPtr tables_end) {
    // Check
    if (virt_addr & (L2_SIZE - 1) || phys_addr & (L2_SIZE - 1) || size & (L2_SIZE - 1)) return false;
...
bool TableManager::map_region(uPtr virt_start, uPtr phys_start, uSize size, PageAttributes attrs,
    ...
    if (virt_start & PAGE_OFFSET_MASK || phys_start & PAGE_OFFSET_MASK || size & PAGE_OFFSET_MASK) {
        DBG::dbgln("Failed to map: virt={:Xl}; phys={:Xl}; size={:X}"_sv, virt_start, phys_start, size);
        return false;
    }
...
bool TableManager::map_upper(u8* table, uPtr& virt_start, uPtr& phys_start, uSize& size, u64 flags, TableLevel level) {
    VERIFY(level != L3);
    ...
    for (; idx < PT_ENTRY_COUNT && size > 0; idx++) {
        ...
        if (entry_kind == PT_INVALID_DESCRIPTOR) {
            ...
            if ((level == L1 || level == L2) && size >= SIZES[level] && (virt_start & (SIZES[level] - 1)) == 0 &&
                (phys_start & (SIZES[level] - 1)) == 0) {
            ...
            tbl[idx] = ARMv8MMU_UpperEntry::create_table_entry(*mem::kernel_virt_to_phys(next_table));
            ...
            // TODO: Spaces overlap!
            DBG::errln("TODO: Spaces overlap!"_sv);
            return false;
        }
        ...
        if (level == L2) {
            ...
            auto res = map_upper(next_table, virt_start, phys_start, size, flags, static_cast<TableLevel>(level + 1));
            if (!res) return false;
        }
    }
    return true;
}
bool TableManager::map_lower(u8* table, uPtr& virt_start, uPtr& phys_start, uSize& size, u64 flags) {
...
TableManager TableManager::create_user_manager() {
    ...
    // TODO: Error handling?
    bek::memset(root_table->start.ptr, 0, PAGE_SIZE);
