...
bool DWHost::init() {
    ...
    // Connect interrupt
    interruptController.register_handler(interrupt_controller::USB, [this]() {return handle_interrupts(); });
...
bool DWHost::init_host() { 
    ...
    CORE::HW_CFG2::set(hw_config2);

    CORE::RX_FIFO_SIZ::set(RX_FIFO_SIZE);
    CORE::NPER_TX_FIFO_SIZ::set(
        0 | RX_FIFO_SIZE | NPER_TX_FIFO_SIZE << 16
        );
    CORE::HOST_PER_TX_FIFO_SIZ::set(
        0 | (RX_FIFO_SIZE + NPER_TX_FIFO_SIZE) | PER_TX_FIFO_SIZE << 16
        );
...
void DWHost::flush_tx_fifo(u32 n_fifo) {
    auto reset = (CORE::RESET_TX_FIFO_FLUSH & ~CORE::RESET_TX_FIFO_NUM__MASK) | n_fifo << CORE::RESET_TX_FIFO_NUM__SHIFT;
