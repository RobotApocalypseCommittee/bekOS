...
struct __attribute__((__packed__)) UsbDeviceRequest {
    u8 Type;													// +0x0
    DeviceRequestCode Request : 8;													// +0x1
    u16 Value;													// +0x2
    u16 Index;													// +0x4
    u16 Length;												// +0x6
};
...
struct __attribute__((__packed__)) UsbDescriptorHeader {
    ...
    usb_descriptor_type DescriptorType : 8;					// +0x1
};
...
struct usb_device_descriptor {
    u8  bLength;												// +0x0 Length of this descriptor
    u8  bDescriptorType;										// +0x1 Descriptor type
    u16 bcdUSB;												// +0x2 (in BCD 0x210 = USB2.10)
    u8  bDeviceClass;											// +0x4 Class code (enum DeviceClass )
    u8  bDeviceSubClass;										// +0x5 Subclass code (assigned by the USB-IF)
    u8  bDeviceProtocol;										// +0x6 Protocol code (assigned by the USB-IF)
    u8  bMaxPacketSize0;										// +0x7 Maximum packet size for endpoint 0
    u16 idVendor;												// +0x8 Vendor ID (assigned by the USB-IF)
    u16 idProduct;												// +0xa Product ID (assigned by the manufacturer)
    u16 bcdDevice;												// +0xc Device version number (BCD)
    u8  iManufacturer;											// +0xe Index of String Descriptor describing the manufacturer.
    u8  iProduct;												// +0xf Index of String Descriptor describing the product
    u8  iSerialNumber;											// +0x10 Index of String Descriptor with the device's serial number
    u8  bNumConfigurations;									// +0x11 Number of possible configurations
} __attribute__((__packed__));


struct __attribute__((__packed__, aligned(1))) configuration_characteristics {
    u8 _reserved0_4 : 5;
    u8 RemoteWakeup : 1;
    u8 SelfPowered  : 1;
    enum {
...
struct usb_configuration_descriptor {
    u8  bLength;												// +0x0 Length of this descriptor
    u8  bDescriptorType;										// +0x1 DEVICE descriptor type(enum DescriptorType)
    u16 wTotalLength;											// +0x2 Total length of all descriptors for this configuration
    u8  bNumInterfaces;										// +0x4 Number of interfaces in this configuration
    u8  bConfigurationValue;									// +0x5 Value of this configuration (1 based)
    u8  iConfiguration;										// +0x6 Index of String Descriptor describing the configuration
    configuration_characteristics attributes;
    u8  bMaxPower;												// +0x8 Maximum power consumed by this configuration
} __attribute__((__packed__));
...
struct __attribute__((__packed__)) UsbOtherSpeedConfigurationDescriptor {
    ...
    u16 TotalLength;											// +0x2 Total length of all descriptors for this configuration
    u8 InterfaceCount;											// +0x4 Number of interfaces in this configuration
    u8 ConfigurationValue;										// +0x5 Value of this configuration (1 based)
    u8 StringIndex;											// +0x6 Index of String Descriptor describing the configuration
    configuration_characteristics Attributes;													// +0x7 Configuration characteristics
    u8 MaximumPower;											// +0x8 Maximum power consumed by this configuration
};
...
struct __attribute__((__packed__)) UsbInterfaceDescriptor {
    struct UsbDescriptorHeader Header;    							// +0x0 Length of this descriptor, +0x1 DEVICE descriptor type (enum DescriptorType)
    u8 Number;													// +0x2 Number of this interface (0 based).
    u8 AlternateSetting;										// +0x3 Value of this alternate interface setting
    u8 EndpointCount;											// +0x4 Number of endpoints in this interface
    InterfaceClass Class : 8;													// +x05 Class code (assigned by the USB-IF)
    u8 SubClass;												// +x06 Subclass code (assigned by the USB-IF)
    u8 Protocol;												// +x07 Protocol code (assigned by the USB-IF)
    u8 StringIndex;											// +x08 Index of String Descriptor describing the interface
};
...
struct __attribute__((__packed__)) UsbEndpointDescriptor {
    ...
    u8 Interval;												// +0x6 Polling interval in frames
};
...
struct __attribute__((__packed__)) UsbStringDescriptor {
    struct UsbDescriptorHeader Header;								// +0x0 Length of this descriptor, +0x1 DEVICE descriptor type (enum DescriptorType)
    char16 Data[];											// +0x2 Amount varies with string length
};

template<uSize N>
struct __attribute__((__packed__, aligned(4))) UsbStringDescriptorStatic {
    ...
    union {
        struct __attribute__((__packed__, aligned(1))) {
            union {
                ...
                u16 RawStatus;									// The same 16 bit status as raw bits
            };
            union {
                struct HubStatusChange Change;						// 16 bit change status as a hub port chnage structure
                u16 RawChange;									// The same 16  bit change status as raw bits
            };
        };
        u32 Raw32;												// Both status joined as one raw 32 bits
    };
};
...
struct __attribute__((__packed__)) HidDescriptor {
    ...
    union {															// Place a union over BCD version .. alignment issues on ARM7/8
        struct __attribute__((__packed__, aligned(1))) {
            u8 HidVersionLo;									// Lo of BCD version
            u8 HidVersionHi;									// Hi of BCD version
        };
    ...
    union {															// Place a union over length .. alignment issues on ARM7/8
        struct __attribute__((__packed__, aligned(1))) {
            u8 LengthLo;										// Lo of Length
            u8 LengthHi;										// Hi of Length
        };
        u16 Length;											// +0x7
    };
};
...
struct UsbDevice {
    ...
    u8 MaxInterface ALIGN4;					// Maxiumum interface in array (varies with config and usually a lot less than the max array size)
    ...
    usb_device_descriptor Descriptor ALIGN4;	// Device descriptor it's accessed a bit so we have a copy to save USB bus ... align it for ARM7/8
...
struct HidDevice {
    ...
    u8 HIDInterface[MaxHIDPerDevice];				// The interface the HID descriptor is on
    u8 MaxHID ALIGN4;								// Maxiumum HID in array (usually less than the max array size) .. align it for ARM7/8
};
...
/*-HCDGetDescriptor ---------------------------------------------------------
 ...
 24Feb17 LdB
 --------------------------------------------------------------------------*/
Result HCDGetDescriptor (const struct UsbPipe pipe,					// Pipe structure to send message thru (really just u32)
                         ...
                         u32 *bytesTransferred,				// Value at pointer will be updated with bytes transfered to/from buffer (NULL to ignore)
...
/*-IsMassStorage------------------------------------------------------------
 ...
 24Feb17 LdB
 --------------------------------------------------------------------------*/
bool IsMassStorage (u8 devNumber);
...
/*-UsbDeviceAtAddress -------------------------------------------------------
 ...
 11Apr17 LdB
 --------------------------------------------------------------------------*/
UsbDevice *UsbDeviceAtAddress (u8 devNumber);
...
/*- HIDReadDescriptor ------------------------------------------------------
 ...
 23Mar17 LdB
 --------------------------------------------------------------------------*/
Result HIDReadDescriptor (u8 devNumber,						// Device number (address) of the device to read
...
/*- HIDReadReport ----------------------------------------------------------
 ...
 23Mar17 LdB
 --------------------------------------------------------------------------*/
Result HIDReadReport (u8 devNumber,							// Device number (address) of the device to read
                      u8 hidIndex,								// Which hid configuration information is requested from
...
/*- HIDWriteReport ----------------------------------------------------------
 ...
 23Mar17 LdB
 --------------------------------------------------------------------------*/
Result HIDWriteReport (u8 devNumber,							// Device number (address) of the device to write report to
...
/*- HIDSetProtocol ----------------------------------------------------------
 ...
 23Mar17 LdB
 --------------------------------------------------------------------------*/
Result HIDSetProtocol (u8 devNumber,							// Device number (address) of the device
