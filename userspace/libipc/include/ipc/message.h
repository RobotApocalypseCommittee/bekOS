...
public:
    ...
    MessageBuffer(u32 message_id, bek::span<u8> data, bek::span<long> fds,
                  bek::span<bek::pair<void*, uSize>> memory_regions);

    const sc::interlink::MessageHeader& header() const {
        return *reinterpret_cast<const sc::interlink::MessageHeader*>(m_buffer.data());
    }
    ...
    bek::span<u8> data_at(uSize offset, uSize size) const {
        return bek::span{m_buffer.data() + offset, m_buffer.data() + offset + size};
    }
...
public:
    ...
    void encode(const T& o) {
        Serializer<T>::encode(o, *this);
    }

    template <Serializable T>
    core::expected<T> decode() {
        return Serializer<T>::decode(*this);
    }
    ...
    constexpr u32 message_id() const { return m_message_id; }
    constexpr uSize data_size() const { return m_buffer.size(); }

private:
    Message(u32 message_id, bek::vector<u8> m_buffer, bek::vector<long> m_fds,
...
struct enum_traits {
    static u32 from_enum(T v) { return static_cast<__underlying_type(T)>(v); }
